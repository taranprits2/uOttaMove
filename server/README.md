...
### Robust Connectivity and Weighted Scoring Improvements

#### [MODIFY] [walking_router.js](file:///c:/Users/taran/uOttaMove/server/src/services/routing/walking_router.js)
- Implement length-weighted accessibility percentage calculation.
- Increase fuzzy welding radius to 3.0m to handle larger OSM gaps.
- Increase snapping search radius and candidate count for better reachability.
- Add "Bridge Edges" discovery to connect isolated graph components that are very close (within 5m).

### Frontend Updates
- Add tooltips or details to explain the accessibility score components.
- Improve error messaging for "No path found" with suggestions.
